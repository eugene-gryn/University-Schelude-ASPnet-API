...
public class UserController : ControllerBase {
    ...
    /// <summary>
    ///     Gets all list of users (id admin)
    ///     Gets list of own user (if user)
    /// </summary>
    ...
    /// <summary>
    ///     Change property admin for other user (Function is for admins only!)
    /// </summary>
    ...
    /// <exception cref="ExceptionModelBase">404 NotFound (user-not-found) - If user id is not related to any entity</exception>
    /// <exception cref="ExceptionModelBase">401 Unauthorized (wrong-refresh-token) - If provided refresh token is not correct</exception>
    [HttpPatch("user/password")]
    public async Task<ActionResult<bool>> ChangePassword(int? id, string? old, [Required] string renew) {
        try {
    ...
    /// <exception cref="ExceptionModelBase">401 Unauthorized (wrong-token) - If token parsing get wrong!</exception>
    /// <exception cref="ExceptionModelBase">
    ///     403 Forbidden (invalid-operation-access-to-user) - If user try to delete other
    ///     user profile!
    /// </exception>
